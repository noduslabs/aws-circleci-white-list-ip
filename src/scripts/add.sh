...
Add() {
  ...
  ${AWS_COMMAND} ec2 authorize-security-group-ingress --group-id "${PARAM_GROUPID}" --ip-permissions \
    '$(echo '[{"IpProtocol": "tcp", "FromPort": 443, "ToPort": 443, "IpRanges": [{"CidrIp": "", "Description": "CircleCI"}]}]' | jq -c '.[].IpRanges[].CidrIp="'""${IP}""/""${PARAM_MASK}""'"|.[].IpRanges[].Description="'""${PARAM_DESC}""'"|.[].FromPort='"${PARAM_PORT}"'|.[].ToPort='"${PARAM_PORT}"'')'
}
